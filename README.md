...
Text Generated by Temperature Sampling:
	...
	temperature:  1.2
	 ...
	 he talked to her in sheers pkpsive instlads or peh from the seen me wehe and redecen entorated of half never
...
From above, we can see that our model achieve a relatively small loss, high accuracy and very fast training speed. This is because we use tensor flow to processing data. Moreover, we do not use sequential model. So it can produce some meaningful text, not like the first model. Besides, we can also use different sampling method to generate text. The key is that never use sequential model. As for the reasons, we need to do further research about that.
